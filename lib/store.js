/* ============================================================
   ...
   Three modes, chosen automatically (highest priority first):
   1. "postgres": when DATABASE_URL is set (.env locally, Vercel
      environment variable in production). Database, products and
      images all live in Postgres (Neon) — tables auto-created,
      fresh database auto-seeded. See lib/db.js.
   2. "blob"  (Vercel): when BLOB_READ_WRITE_TOKEN (or BLOB_STORE_ID)
      is set, the database is stored at `alwazir/db.json` (private)
      and uploads at `alwazir/uploads/<name>` (public) in Vercel Blob.
   3. "local" (default): one JSON file `data/db.json` + uploads in
      `public/uploads/`. Works anywhere with a writable disk.
   ============================================================ */
...
// Font ids live in public/js/fonts.js (single source of truth, shared
// with the admin panel) — see FONTS there.
...
function storageMode() {
  if (pgdb.configured()) return 'postgres';
...
async function getData() {
  if (storageMode() === 'postgres') {
    try {
      const raw = await pgdb.fetchDb();
      if (Object.keys(raw.settings).length === 0 && raw.products.length === 0) {
        // Fresh, empty database — seed it with the default store,
        // mirroring what local mode does for a missing db.json.
        const seeded = deepCopy(DEFAULT_DB);
        try {
          await pgdb.saveDb(seeded);
        } catch (e) {
          console.warn('Could not seed the Postgres database:', e.message);
        }
        db = seeded;
        cacheAt = Date.now();
        return seeded;
      ...
      // Unreachable/misconfigured database: keep the store usable
      // in-memory while /api/status surfaces the real problem.
      console.warn('Could not read database from Postgres:', e.message);
...
async function saveData(nextDb) {
  ...
  if (storageMode() === 'postgres') {
    try {
      await pgdb.saveDb(nextDb);
    } catch (e) {
      // Throw so the admin panel shows an error instead of silently reverting.
      throw new Error('Could not save to the Postgres database: ' + e.message);
    }
    return;
  }
...
async function listUploads() {
  if (storageMode() === 'postgres') {
    try {
      return await pgdb.listUploadsRows();
    } catch (e) {
      return [];
    }
  }
...
async function pruneUploads(limit) {
  ...
  for (const u of uploads) {
    if (excess <= 0) break;
    try {
      if (storageMode() === 'postgres') await pgdb.deleteUploadRow(u.name);
      else if (storageMode() === 'blob') await blobClient().del(u.url);
...
async function saveUpload(name, buffer, contentType) {
  ...
  if (storageMode() === 'postgres') {
    try {
      await pgdb.saveUploadRow(name, buffer, contentType || 'application/octet-stream');
      return '/uploads/' + name;
    } catch (e) {
      throw new Error('Could not upload image to Postgres: ' + e.message);
    }
  }
  if (storageMode() === 'blob') {
...
async function getUpload(name) {
  return pgdb.getUploadRow(name);
}

/* Connectivity probe for the Postgres database (used by /api/status). */
